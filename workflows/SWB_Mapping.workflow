...
Select and define a proper reference system.
...
!INSTRUCTIONS
.ALGORITHM:otb:bandmath
.PARAMETERS:{"-ram": 128, "-exp": "im1b1+200"}
.MODE:Normal
...
Expression:
im1b1+200
(factor adjustment by e.g. value "200")
...
.PARAMETERS:{"formula": "if((A>=150),1,0)", "GRASS_REGION_CELLSIZE_PARAMETER": 0}
.MODE:Normal
.INSTRUCTIONS:In this step the first intermediate water bodies (v1) are derived by considering a threshold greater or equal 150 of the values, gained from the combination of indices.
...
Within step 6, the first intermediate water bodies (v1) are derived by considering a threshold greater or equal 150 of the values, gained from the combination of indices. The given value is not static and should always be compared with the results of Step 5 (05_COMBINED_INDICES_[NAME].tif) in order to select and define the best possible threshold.
!INSTRUCTIONS
.ALGORITHM:modeler:swb_02_clump_and_sieve_iteration_1
.PARAMETERS:{"NUMBER_MMUPIXEL": 1}
.MODE:Normal
...
!INSTRUCTIONS
.ALGORITHM:grass:r.mapcalculator
.PARAMETERS:{"formula": "if((A>=130 && B>0),1,C)", "GRASS_REGION_CELLSIZE_PARAMETER": 0}
.MODE:Normal
...
!INSTRUCTIONS
.ALGORITHM:grass:r.mapcalculator
.PARAMETERS:{"formula": "if((A>=140 && B>0),1,C)", "GRASS_REGION_CELLSIZE_PARAMETER": 0}
.MODE:Normal
...
!INSTRUCTIONS
.ALGORITHM:grass:r.mapcalculator
.PARAMETERS:{"formula": "if(A>=120,1,0)", "GRASS_REGION_CELLSIZE_PARAMETER": 0}
.MODE:Normal
...
!INSTRUCTIONS
.ALGORITHM:grass:r.mapcalculator
.PARAMETERS:{"formula": "if(isnull(A),0,A)", "GRASS_REGION_CELLSIZE_PARAMETER": 0}
.MODE:Normal
